...
var _ ipc.Server = &CenterServer{} // It provides a static (compile time) check that CenterServer satisfies the ipc.Server interface.

type Message struct {
	From    string "from" // unused
	To      string "to"   // unused
	...
	for i, v := range server.players {
		if v.Name == params {
			if len(server.players) == 1 {
				...
				server.players = append(server.players[:i-1], server.players[:i+1]...) // append a slice
			}
